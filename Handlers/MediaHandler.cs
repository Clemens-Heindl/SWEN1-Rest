        ...
        if(e.Path.StartsWith("/media"))
        {
            if((e.Path == "/media") && (e.Method == HttpMethod.Post))
            {
                ...
                catch(Exception ex)
                {
                    ...
                    Console.WriteLine($"[{nameof(VersionHandler)} Exception creating media entry. {e.Method.ToString()} {e.Path}: {ex.Message}");
                }
            }
            else              
            if((e.Path == "/media") && (e.Method == HttpMethod.Post))
            ...
            else
            {
                ...
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine($"[{nameof(VersionHandler)} Invalid media entry endpoint.");
            }
